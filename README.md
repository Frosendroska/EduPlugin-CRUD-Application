...
Запустить БД postgresql с именем torrent на порту 5432

Собрать протобуфы: `gw build`
...
* На трекере хранится список файлов и информация об активных пользователях, у которых есть те или иные файлы (возможно
  не целиком).
* С помощью клиентского приложения можно просматривать список файлов на трекере, а также добавлять новые и выбирать
  файлы из списка для скачивания.
* Файлы условно разбиваются на последовательные блоки бинарных данных константного размера (например 10M). Последний
  блок может иметь меньший размер. Блоки нумеруются с нуля.
...
* При скачивании файла клиент получает у трекера информацию о клиентах, раздающих файл (сидах), и далее общается с ними
  напрямую.
...
* **Важно:** перечисленные выше запросы являются _рекомендованными_, как и их формат, описываемый ниже. Вы можете менять
  список и семантику запросов на своё усмотрение (в случае таких изменений вам требуется где-то задокументировать какие
  запросы есть и для чего используются (можно в комментариях в `.proto`)).
    * Реализовывать (де)сериализацию запросов вы можете как вручную, так и с использованием любых библиотек и
      фреймворков
...
* Если клиент А и клиент Б решили опубликовать файл abc.txt, то это будут **разные** файлы, иными словами каждый запрос
